...
class PropertiesController extends Controller
{
    ...
    {
        $images = PropertiesImage::find()->where(['PropertiesID'=>"$id"])->one();

        $image=Images::findone($images->ImagesID);

        
        $myPath = $_SERVER['DOCUMENT_ROOT'];
        $halfPaht = strrchr($image->ImagePath,"yii2");
        $image->ImagePath='https://t7187321.scm.tees.ac.uk'.'/'.$halfPaht;

        return $this->render('view', [
            'model' => $this->findModel($id),'images'=>$images,'image'=>$image
        ]);
    }
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect(['view', 'id' => $model->PropertiesID]);
        } else {
    ...
    {
        $propertiesImage = PropertiesImage::findOne($id);
        $imagesModel = new Images();
        $imagesModel->ImagesID=propertiesImage->ImagesID;
        propertiesImage->delete();
        imagesModel->findModel( $imagesModel->ImagesID)->delete();
